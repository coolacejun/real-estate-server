...
The API applies an in-process IP safety limit to OAuth, store, report and environment calls. Keep an edge or Redis-backed distributed rate limit enabled when running more than one API worker. Request body defaults are 64 KiB for auth, 3 MiB for store, 64 MiB for reports, 16 KiB for environment and 256 KiB for internal calls, 6 MiB per image and eight images per report. Adjust the report limits only through the documented environment variables.
