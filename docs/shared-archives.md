...
1. Publish the reviewed local commits only after separate authorization. The
   server's existing `web` submodule pin is deliberately unchanged; pin the
   approved matching web commit before building a release. Current checkout
   composition alone is not a deployable integrated release.
