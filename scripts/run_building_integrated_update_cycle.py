...
class BuildingIntegratedChangeItem:
    base_date: str
    updated_date: str
    file_no: str
    ds_file_id: str
    ...
    def zip_name(self) -> str:
        return f"CH_D010_00_{self.date_compact}.zip"

    @property
    def geojson_name(self) -> str:
        return f"CH_D010_00_{self.date_compact}.geojson"
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Download missing VWorld building integrated CH files, convert them to GeoJSON, and import updates."
    )
    ...
    parser.add_argument(
        "--api-base",
        default=os.getenv("BUILDING_LAND_API_BASE", os.getenv("BUILDING_INTEGRATED_SYNC_API_BASE", "http://localhost")),
    )
    parser.add_argument("--env-file", default=str(repo_root / ".env"))
    parser.add_argument("--page-count", type=int, default=int(os.getenv("BUILDING_INTEGRATED_SYNC_PAGE_COUNT", "20") or "20"))
    parser.add_argument("--page-size", type=int, default=int(os.getenv("BUILDING_INTEGRATED_SYNC_PAGE_SIZE", "30") or "30"))
    parser.add_argument("--fetch-timeout", type=float, default=float(os.getenv("BUILDING_INTEGRATED_SYNC_FETCH_TIMEOUT", "120") or "120"))
    ...
    parser.add_argument(
        ...
        type=float,
        default=float(os.getenv("BUILDING_INTEGRATED_SYNC_MAX_DIRECT_DOWNLOAD_MB", "500") or "500"),
    )
    ...
    parser.add_argument(
        ...
        default=_truthy_env("BUILDING_INTEGRATED_SYNC_CLEANUP_ON_SUCCESS", default=True),
    )
    parser.add_argument(
        "--start-date",
        default=os.getenv("BUILDING_INTEGRATED_SYNC_START_DATE", ""),
...
def parse_change_items(html: str) -> list[BuildingIntegratedChangeItem]:
    rows: list[BuildingIntegratedChangeItem] = []
    parts = re.split(r'<li><!--v-for="n in 10"-->|<li>', html)
    for part in parts:
        if "GIS건물통합정보" not in part or "구분<em>변동데이터</em>" not in part:
            continue
...
def discover_change_items(opener: urllib.request.OpenerDirector, args: argparse.Namespace, lower_bound: str) -> list[BuildingIntegratedChangeItem]:
    found: list[BuildingIntegratedChangeItem] = []
...
def active_release_and_recorded(args: argparse.Namespace) -> tuple[dict[str, Any], set[str]]:
    ...
    with psycopg.connect(db_url) as conn:
        with conn.cursor() as cur:
            ...
            row = cur.fetchone()
            if not row:
                raise RuntimeError("active building_integrated_info release not found")
            release = {
                "id": int(row[0]),
                "version": row[1],
                "source_name": row[2],
            ...
            cur.execute(
                ...
                WHERE release_id = %s
                  AND data_type = 'building_integrated_info'
                """,
                (int(release["id"]),),
            )
...
def date_from_update_file(name: str) -> str:
    match = re.search(r"CH_D010_00_(20\d{6})", str(name or ""))
    if not match:
        return ""
...
def download_zip(
    opener: urllib.request.OpenerDirector,
    item: BuildingIntegratedChangeItem,
    raw_dir: Path,
    timeout: float,
    force: bool,
) -> Path:
    ...
    if target.exists() and target.stat().st_size > 0 and not force:
        if zipfile.is_zipfile(target):
            return target
        print(f"[building-integrated-cycle] remove invalid cached zip: {target}", flush=True)
...
def convert_zip_to_geojson(zip_path: Path, item: BuildingIntegratedChangeItem, extracted_root: Path, output_dir: Path, force: bool) -> Path:
...
def start_import(api_base: str, token: str, stage_dir: Path, items: list[BuildingIntegratedChangeItem]) -> dict[str, Any]:
    first_date = items[0].base_date
    last_date = items[-1].base_date
    body = {
        "data_type": "building_integrated_info",
        "operation_mode": "update",
        "source_path": str(stage_dir),
        "pattern": "CH_D010*.geojson",
        "source_name": f"VWorld building integrated CH {first_date}..{last_date}",
        "mark_ready": True,
    }
    payload = http_json(
        api_base,
...
def latest_job(api_base: str, token: str, job_id: int) -> dict[str, Any] | None:
    payload = http_json(api_base, "/v1/admin/cadastral/import-jobs?data_type=building_integrated_info&limit=20", token=token, timeout=60)
    data = payload.get("data")
...
def wait_for_import(api_base: str, token: str, job_id: int, timeout: float, poll_interval: float) -> dict[str, Any]:
    ...
    while True:
        ...
        if status != last_status or status == "RUNNING":
            print(
                "[building-integrated-cycle] import "
...
def verify_recorded_update_files(args: argparse.Namespace, expected_file_names: list[str]) -> dict[str, Any]:
    ...
    with psycopg.connect(db_url) as conn:
        with conn.cursor() as cur:
            ...
            if not release_row:
                raise RuntimeError("active building_integrated_info release not found after import")
...
def acquire_cycle_lock(path: Path) -> int | None:
    ...
    try:
        ...
        if pid and not process_exists(pid):
            print(f"[building-integrated-cycle] removing stale cycle lock pid={pid}: {path}", flush=True)
            with contextlib.suppress(Exception):
                path.unlink()
            fd = os.open(str(path), os.O_CREAT | os.O_EXCL | os.O_WRONLY)
            os.write(fd, f"{os.getpid()}\n{dt.datetime.now().astimezone().isoformat(timespec='seconds')}\n".encode("utf-8"))
...
def main() -> int:
    ...
    manifest_name = f"building_integrated_info_ch_{run_id}.json"
    ...
    try:
        ...
        print(
            f"[building-integrated-cycle] active_release={release.get('id')} lower_bound={lower_bound} "
        ...
        for item in missing:
            try:
                print(f"[building-integrated-cycle] download {item.zip_name} fileNo={item.file_no} size={item.size_text}", flush=True)
                zip_path = download_zip(opener, item, dirs["raw"], args.fetch_timeout, args.force)
                print(f"[building-integrated-cycle] convert {zip_path.name} -> {item.geojson_name}", flush=True)
                ...
                print(f"[building-integrated-cycle] failed {item.zip_name}: {exc}", file=sys.stderr, flush=True)
                break
        ...
        manifest["finished_at"] = dt.datetime.now().astimezone().isoformat(timespec="seconds")
        write_manifest(base_dir, manifest_name, manifest)
        print(f"[building-integrated-cycle] failed: {exc}", file=sys.stderr, flush=True)
        return 1
    finally:
