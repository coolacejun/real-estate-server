...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(description="Run cadastral CH update cycle on an interval.")
    parser.add_argument("--repo-root", default=str(repo_root))
    parser.add_argument(
        "--base-dir",
        default=os.getenv("CADASTRAL_SYNC_BASE_DIR", str(repo_root / "data/source/cadastral/auto")),
    )
    parser.add_argument("--api-base", default=os.getenv("BUILDING_LAND_API_BASE", os.getenv("CADASTRAL_SYNC_API_BASE", "http://localhost")))
    parser.add_argument("--env-file", default=str(repo_root / ".env"))
    parser.add_argument("--check-interval", type=float, default=float(os.getenv("CADASTRAL_SYNC_CHECK_INTERVAL_SECONDS", "86400") or "86400"))
    parser.add_argument("--retry-interval", type=float, default=float(os.getenv("CADASTRAL_SYNC_RETRY_SECONDS", "3600") or "3600"))
    parser.add_argument("--initial-delay", type=float, default=float(os.getenv("CADASTRAL_SYNC_INITIAL_DELAY_SECONDS", "0") or "0"))
...
def sleep_with_log(seconds: float, reason: str) -> None:
    ...
    print(
        f"[cadastral-scheduler] sleep {int(safe_seconds)}s until {wake_at.isoformat(timespec='seconds')} reason={reason}",
        flush=True,
    )
...
def run_cycle_once(args: argparse.Namespace) -> int:
    cmd = [
        sys.executable,
        str(Path(__file__).resolve().with_name("run_cadastral_update_cycle.py")),
        "--repo-root",
        args.repo_root,
        "--base-dir",
        args.base_dir,
...
def main() -> int:
    ...
    while True:
        try:
            ...
            print(f"[cadastral-scheduler] failed: {exc}", file=sys.stderr, flush=True)
